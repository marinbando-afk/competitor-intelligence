...
function fmtAds(d, today) {
  ...
  const lines = sample.map((a) => `- [${a.started || '?'}] ${a.hasVideo ? 'VIDEO' : 'IMAGE'} · fb-page:"${a.page || '?'}"${kindTag(a)}${a.cta ? ` · cta:"${a.cta}"` : ''}${a.landing ? ` · landing-page:${adHost(a.landing)}${ff.own(adHost(a.landing)) ? '' : ' (3RD-PARTY)'}` : ''} :: ${oneLine(a.text).slice(0, 170)}`);
