...
function fmtAds(d, today) {
  ...
  const landSeg = (a) => {
    ...
    // 404; /founder is the live page) — the model must cite URLs that actually open.
    let path = '';
    try { path = new URL(a.landing).pathname; if (path === '/') path = ''; } catch (e) { /* domain only */ }
    return ` · landing-page:${dm}${path.slice(0, 40)}${ff.own(dm) ? '' : ' (3RD-PARTY)'}`;
  };
  const lines = sample.map((a) => `- [${a.started || '?'}] ${a.hasVideo ? 'VIDEO' : 'IMAGE'} · fb-page:"${a.page || '?'}"${kindTag(a)}${a.cta ? ` · cta:"${a.cta}"` : ''}${landSeg(a)} :: ${oneLine(a.text).slice(0, 170)}`);
