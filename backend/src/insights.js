...
function fmtAds(d, today) {
  ...
  const landSeg = (a) => {
    ...
    if (!dm) return '';
    if (/^(facebook|fb|instagram)\.com$/i.test(dm)) return ` · landing:their-own-${/instagram/i.test(dm) ? 'Instagram' : 'Facebook'}-page (PAGE-LIKE/follow campaign)`;
