...
function parseOut(txt) {
  ...
  if (!o || typeof o !== 'object') return { summary: oneLine(txt).slice(0, 240), bullets: [] };
  return {
...
async function ask(channel, brand, todayBlock, prevBlock) {
  ...
  const system =
    ...
    `Return ONLY minified JSON, no markdown: {"summary":"<one tight sentence (<=18 words): the single most important or most-new takeaway>","bullets":["<short, specific point>", ...]} with 0–4 bullets. If nothing changed and nothing notable, return a 1-sentence summary and an empty bullets array.`;
  const user = `=== TODAY ===\n${todayBlock}\n\n=== PREVIOUS CAPTURE ===\n${prevBlock && prevBlock.trim() ? prevBlock : '(no earlier capture to compare against yet)'}`;
  const resp = await client().messages.create({ model: MODEL, max_tokens: 600, system, messages: [{ role: 'user', content: user }] });
...
export async function generateInsights(brand, host) {
  ...
  try {
    ...
    if (r[0] && r[0].data) out.ads = await ask('ads', brand, fmtAds(r[0].data), r[1] && r[1].data ? fmtAds(r[1].data) : '');
  } catch (e) { /* skip */ }
  ...
  try {
    ...
    if (r[0] && r[0].data) {
      ...
      out.website = await ask('website', brand, todayBlock, r[1] && r[1].data ? fmtWeb(r[1].data) : '');
    }
  } catch (e) { /* skip */ }

  try {
    const em = await getEmails(host);
    if (em && em.emails && em.emails.length) out.email = await ask('email', brand, fmtEmail(em), '');
  } catch (e) { /* skip */ }
